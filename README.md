...
* ****`divideNumbers(uint256 _firstNumber, uint256 _secondNumber)`****: Divides `_firstNumber` by `_secondNumber` and returns the quotient. Includes a validation check to ensure the second number is greater than zero to avoid
